...
class RpTest(unittest.TestCase):
    ...
    def test_create_retention_policy_empty_db_parameter(self):
        ...
        with self.assertRaises(ValueError):
            cli.create_retention_policy(dbname='', rp_config=rp_config,
                                        is_default=True)
            cli.close()
    ...
    def test_create_retention_policy_success(self):
        ...
        qr = cli.create_retention_policy(dbname=dbname,
                                         rp_config=rp_config,
                                         is_default=True)
    ...
    def test_show_retention_policies_success(self):
        ...
        cli = test_utils.get_test_default_client()
        cli.create_database(dbname)
        cli.create_retention_policy(dbname=dbname,
                                    rp_config=rp_config,
                                    is_default=True)
        retention_polices = cli.show_retention_policies(dbname=dbname)
        rp_config_name = [rp[0] for rp in retention_polices]
    ...
    def test_show_retention_policy_error(self):
        cli = test_utils.get_test_default_client()
        with self.assertRaises(ValueError):
            cli.show_retention_policies('')
        cli.close()
